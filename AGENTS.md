...
- **`HasUserScope`** concern: global scope `where('user_id', auth()->id())` for non-admin — on Persona, Vehiculo, Contrato, ControlDiario, Deuda
...
- Admin-only resources: ActivityLogResource (Trazabilidad), UserResource (Usuarios)
